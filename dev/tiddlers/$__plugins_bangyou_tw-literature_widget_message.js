...
(function () {
    ...
    */
    MessageWidget.prototype.render = function (parent, nextSibling) {
        ...
        containerDom.addEventListener("research-message", function (event) {
            ...
            if (request.method === "open_tiddler") {
                ...
                var isSaveCanonical = $tw.wiki.getTiddlerText("$:/config/tw-literature/SaveCanonicalTiddler") || "disable";
                ...
                // copy image macro
                var copyImgMacro = $tw.wiki.getTiddlerText("$:/config/tw-literature/CopyImageMacro") || " [img[$filename$]] ";
                if (copyImgMacro !== "") {
                    const img_macro = copyImgMacro.replace("$filename$", filename);
                    copyToClipboard(img_macro);
                    var InsertIntoTextArea = $tw.wiki.getTiddlerText("$:/config/tw-literature/InsertIntoTextArea") || "disable";
    ...
    function getTimestampedFilename(ext) {
        var format = $tw.wiki.getTiddlerText("$:/config/tw-literature/ImageFileFormat") || "images/YYYY/YYYY0MM0DD0hh0mm0ss";
        const now = new Date();
